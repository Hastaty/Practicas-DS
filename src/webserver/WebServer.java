...
public class WebServer {
  ...
  private Activity findActivityById(int id) {
    SearcherById searcher = new SearcherById(id);
    ...
    private String makeBodyAnswer(String[] tokens) {
      ...
      switch (tokens[0]) {
        ...
        case "newAct":{
          ...
          String tag=tokens[2].toString();
          int parentId = Integer.parseInt(tokens[3]);
          if(tokens[4].toString().equals("Task"))//WE WANT TO CREATE A TASK
          {
            Task newact = new Task(name);
            newact.setTag(tag);
            SearcherById parentSearcher= new SearcherById(parentId);
            root.accept(parentSearcher);
            Project parentpr = (Project)parentSearcher.getObjList().get(0);
            newact.setParentProject(parentpr);
            parentpr.addActivity(newact);
          }
          else                              //WE WANT TO CREATE A PROJECT
          {
            Project newact = new Project(name);
            newact.setTag(tag);
            SearcherById parentSearcher= new SearcherById(parentId);
            root.accept(parentSearcher);
            Project parentpr = (Project)parentSearcher.getObjList().get(0);
